# MAIC comparing Ra223+ENZ vs Pluvicto (comparator trial: Satapathy)
...
# Import reconstructed IPD from Satapathy
comparator_ipd <- list()
comparator_ipd$OS <- read_csv(here(km_directory, "OS_Satapathy_PLU.csv"))
comparator_ipd$PFS <- read_csv(here(km_directory, "PFS_Satapathy_PLU.csv"))

# Load comparator baseline data
comparator_baseline <- as.data.frame(read_xlsx(here("data", "baseline_Satapathy.xlsx"))) #note: ECOG in Satapathy include ECOG 0-1
...
# 5. Export results --------------------------------------------------------
write.csv(results_table, here("results", "maic_results_Satapathy.csv"))  
