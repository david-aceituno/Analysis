# MAIC comparing Ra223+ENZ vs Pluvicto (comparator trial: TheraP)
...
# Import reconstructed IPD from TheraP
comparator_ipd <- list()
comparator_ipd$OS <- read_csv(here(km_directory, "OS_TheraP_PLU.csv"))
comparator_ipd$PFS <- read_csv(here(km_directory, "PFS_TheraP_PLU.csv"))

# Load comparator baseline data
comparator_baseline <- as.data.frame(read_xlsx(here("data", "baseline_TheraP.xlsx"))) #note: ECOG in TheraP include ECOG 0-1
...
# 5. Export results --------------------------------------------------------
write.csv(results_table, here("results", "maic_results_TheraP.csv"))  
